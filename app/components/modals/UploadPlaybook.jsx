...
const UploadPlaybook = ({ onClose }) => {
  const onDrop = useCallback((acceptedFiles) => {
    // Handle the playbook files here
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-[#2E2E2E] p-6 rounded-md shadow-lg relative w-full max-w-md">
        ...
        <h2 className="text-xl mb-4 text-white">Upload Playbook</h2>
        <div
          {...getRootProps()}
        ...
        >
          <input {...getInputProps()} />
          {isDragActive ? (
            <p className="text-white">Drop the playbook files here...</p>
          ) : (
            <p className="text-gray-300">
              Drag & drop playbook files here, or click to select files
            </p>
          )}
        </div>
      </div>
